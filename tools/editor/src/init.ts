...
const AZURE_ALIAS_SPECS: IconAliasSpec[] = [
  ...
  { alias: 'blob-storage', tokenVariants: [['blob', 'storage']], preferredPathTokens: ['icon', 'service'] },
  ...
  { alias: 'key-vault', tokenVariants: [['key', 'vault']], preferredPathTokens: ['icon', 'service'] },
  ...
  if (options.downloadIcons) {
    ...
    for (const config of ICON_PACKAGES) {
      try {
        ...
        console.error(`  Failed to download ${config.name} icons: ${(error as Error).message}`)
      }
    }
