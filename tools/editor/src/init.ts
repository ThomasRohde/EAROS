...
interface AwsIconAliasSpec {
  alias: string
  tokenVariants: string[][]
  preferredPathTokens: string[]
}

const AWS_ICON_ALIAS_SPECS: AwsIconAliasSpec[] = [
...
async function resolveAwsIconPackageUrl(): Promise<string> {
  if (AWS_ICON_PACKAGE_URL) return AWS_ICON_PACKAGE_URL
...
function scoreAliasCandidate(entry: ExtractedIconEntry, spec: AwsIconAliasSpec): number {
  ...
  if (spec.preferredPathTokens.every((token) => entry.pathTokens.has(token))) score += 40
  if (entry.fileTokens.has('arch')) score += 5
  score -= entry.normalizedPath.length / 1000
  return score
}

function createAwsIconAliases(iconsDir: string, extractedEntries: ExtractedIconEntry[]): { aliasCount: number; missingAliases: string[] } {
...
async function downloadAwsIcons(targetDir: string): Promise<{ packageUrl: string; fileCount: number; aliasCount: number; missingAliases: string[] }> {
