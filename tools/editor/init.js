...
function writeVersionMarker(target) {
    const { name, version } = getPackageInfo();
    const content = `# Auto-generated by earos init/update — do not edit.\n` +
...
function readVersionMarker(target) {
 ...
 */
function classifyGovernedPath(relPath, assetsDir, target, workspaceExisted) {
...
function printDiff(src, dest, rel) {
...
export async function updateWorkspace(targetDir, options = {}) {
    const target = resolve(process.cwd(), targetDir);
    const assetsDir = join(__dirname, 'assets', 'init');
    ...
    if (interactive && !input.isTTY) {
        console.error('earos update is interactive by default and stdin is not a TTY.\n' +
    ...
    if (adds.length) {
        ...
        for (const a of adds)
            console.log(`  + ${a.rel.split(sep).join('/')}`);
    }
    ...
    async function resolveOne(c) {
        ...
        if (options.overwrite)
            ...
            choice = c.kind === 'restore' ? 'skip' : 'keep';
        else
            choice = await promptResolution(c.rel, c.src, c.dest, c.kind);
        if (choice === 'overwrite') {
