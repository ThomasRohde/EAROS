...
const _artifactSchemaCache = new Map();
function isSupportedArtifactType(artifactType) {
    return typeof artifactType === 'string' && artifactType in ARTIFACT_TYPE_TO_SCHEMA;
}
function unsupportedArtifactTypeError(artifactType) {
    ...
    return new Error(`Unsupported artifact_type for DOCX export: ${shown}. Expected one of: ${supported}`);
}
function loadArtifactSchema(artifactType) {
    if (_artifactSchemaCache.has(artifactType))
        return _artifactSchemaCache.get(artifactType) ?? null;
...
function renderQualityAttributes(data, children, root) {
    children.push(h1('Quality Attributes'));
    const items = Array.isArray(data) ? data : Object.values(data);
    if (!items.length)
        return;
    const table = objectTable(items.filter((item) => item && typeof item === 'object' && !Array.isArray(item)), itemSchema(sectionSchema(root, 'quality_attributes')));
    if (table)
        children.push(table);
}
function renderOperationalModel(data, children, root) {
...
function renderGovernance(data, children, root) {
...
export async function exportToDocx(artifactData, browserRenderedDiagrams) {
    ...
    const renderSectionByKey = (key, value) => {
    ...
    for (const key of sectionOrder) {
        if (key in sections)
            renderSectionByKey(key, sections[key]);
    }
